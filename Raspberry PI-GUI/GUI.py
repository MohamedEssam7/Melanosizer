...
def obj_araise(frame, isFirst = False):
    if isFirst == False:
...
#
#                 frame = F(parent=container, controller=window,image=img)
#
...
#                 frame.grid(row=0, column=0, sticky="nsew")
#             else:
#                 frame = F(parent=container, controller=window)
#
#                 self.frames[page_name] = frame
#
#                 # put all of the pages in the same location;
...
#
#     def assign_capimage(self,image, parent):
#         frame = Capture(Capture,parent=parent, controller=window,image=image)
#
#         # self.frames[page_name] = frame
#
#         # put all of the pages in the same location;
...
class StartPage(tk.Frame):
    ...
    def __init__(self,parent):
        ...
        label = tk.Label(window, text="Welcome Doctor !",
                         bg="steelblue", fg="white", font=('Comic Sans MS', 20))

        label.pack(side="top", fill="x", pady=10)

        camera_button = tk.Button(window, text="Camera",

                                  command=lambda: obj_araise(Camera(create_container())), bg="LIGHTBLUE", font=('Comic Sans MS', 15), width=20)

        history_button = tk.Button(window, text="History",

                                   command=lambda: obj_araise(History(create_container(),window)), bg="LIGHTBLUE", font=('Comic Sans MS', 15), width=20)
...
#         image_name = datetime.now().strftime('%d-%m-%Y %H-%M-%S')
    ...
    def __init__(self, parent):
        ...
        feedlabel = Label(window, bg="steelblue", fg="white",
                          text="WEBCAM FEED", font=('Comic Sans MS', 20))
        feedlabel.pack(side="top", fill="x", pady=3)

        cameraLabel = Label(window, bg="steelblue",
                            borderwidth=3, relief="groove")
        cameraLabel.pack(side="top", pady=3)
        
        def delay():
            Camera.assign_image(Camera.Capture(self))
            obj_araise(Capture(create_container(), window,Captured_image))
            
        captureBTN = Button(window, text="CAPTURE",  command=lambda: [delay()],
        ...
        back_button = tk.Button(window, text="Back",
                                command=lambda: obj_araise(StartPage(create_container(), window)), bg="red", font=('Comic Sans MS', 15), width=10)
...
class History(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        imageLabel = Label(window, bg="steelblue",
                           borderwidth=3, relief="groove")
        imageLabel.pack(side="top", pady=3)

        openImageButton = Button(
            window, text="BROWSE", command=imageBrowse, bg="LIGHTBLUE", font=('Comic Sans MS', 15), width=20)
        openImageButton.pack(side="left", pady=3)

        camera_button = tk.Button(
            window, text="Camera", command=lambda: obj_araise(Camera(create_container(), window)), bg="LIGHTBLUE", font=('Comic Sans MS', 15), width=20)
...
class Capture(Camera,tk.Frame):
    ...
    def __init__(self, parent, controller, image):
        ...
        capture_button = tk.Button(
            window, text="Capture again !", command=lambda: [obj_araise(Camera(create_container(), window))], bg="LIGHTBLUE", font=('Comic Sans MS', 15), width=20)
        capture_button.pack(side="left", pady=3)

        send_button = tk.Button(
            window, text="Send", command=lambda: obj_araise(Send(create_container(), window)), bg="LIGHTBLUE", font=('Comic Sans MS', 15), width=20)
...
if __name__ == "__main__":
    ...
    window.title("Melanoziser")
    window.state('zoomed')
    obj_araise(StartPage(create_container()),True)
    window.mainloop()
